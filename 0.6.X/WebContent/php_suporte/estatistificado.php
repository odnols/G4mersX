...
<head>
	...
	<!-- Cabeçalho da página -->
	<title>Estatísticas de <?php if($twonumbernine){ echo "$twonumbernine";}else if($data_convertida){echo "$data_convertida";}else{echo "$ano ";} ?></title>
...
<div id="fundo">
	<nav id="menu" class="navbar navbar-default navbar-fixed-top">
      ...
      <a class="nav navbar-nav navbar-right" href="#mes" style="color: white; text-decoration: none"><h5 id="hora">Dados d<?php if($twonumbernine){ echo "o $twonumbernine";}else if(strlen($data) == 4){ echo "o Ano de ".$data; }else{echo "e $mes[$nome_mes]"; } if($ano_convertido != $date3){ echo " de ". $ano_convertido ;}?> ][ Agora são <em id="relogio"></em> de <em><?php echo "$date2" ?></em></h5></a>
      <a class="nav navbar-nav navbar-right" href="#"><h5 id="total_back"><?php echo "Total de mensagens enviadas: <em id='num' style='color: yellow'>$total</em>" ?></h5></a>
	...
	?>
	<?php if($data != 1000 ){ ?>
	<div id="download_automatic"><h3 id="titulo" class="check_box_download">Download automático<input type="checkbox" id="check_add" onclick="atualiza_session()" <?php echo $checkbox; ?>></h3></div>
	<?php } ?>
  	</div>
  	</nav>
	...
	<?php
		...
		<div id="alinhadeira_exp2">
			...
			<?php } if($impares > 0){ ?>
			<hr><h2 class="impares"><em>“N° de dias Ímpares”</em></h2><?php
	...
	if($maior != 0 ){
		if($maior > 1){
			?><br><h2 id="msgs_dias"><em style='color: #0CDD0C' id='num'><?php echo $maior ?></em> Dias passaram de <em class='datas_' id="num">100</em><br> Mensagens <br><div id="alinhadeira_exp2"><?php if($maior > 0){ ?><button id="cima_mais" class="lista_dias_bons" onclick="maior_lista()">⤵</button><button id="baixo_mais" class="lista_dias_bons">⤴</button> <?php } ?> </div></h1><?php
		}else{
			?><br><h2 id="msgs_dias">Apenas <em style='color: #0CDD0C' id='num'><?php echo $maior ?></em> dia passou<br> de <em class='datas_' id="num">100</em> Mensagens <br><div id="alinhadeira_exp2"><?php if($maior > 0){ ?><button id="cima_mais" class="lista_dias_bons" onclick="maior_lista()">⤵</button><button id="baixo_mais" class="lista_dias_bons">⤴</button> <?php } ?> </div></h1><?php
		}
	}else{
		...
		if($menor != 0 ){
			if($menor > 1){
				?><br><h2 id="msgs_dias"><em style='color: red' id='num'><?php echo $menor ?></em> Dias não passaram de <br><em class='datas_' id="num">100</em> Mensagens <br><div id="alinhadeira_exp2"><?php if($maior > 0){ ?><button id="cima_menos" class="lista_dias_ruins">⤵</button><button id="baixo_menos" class="lista_dias_ruins">⤴</button> <?php } ?> </div></h1><?php
			}else{
				?><br><h2 id="msgs_dias">Apenas <em style='color: red' id='num'><?php echo $menor ?></em> dia não passou<br> de <em class='datas_' id="num">100</em> Mensagens <br><div id="alinhadeira_exp2"><?php if($maior > 0){ ?><button id="cima_menos" class="lista_dias_ruins">⤵</button><button id="baixo_menos" class="lista_dias_ruins">⤴</button> <?php } ?> </div></h1><?php
			}
		}else{
