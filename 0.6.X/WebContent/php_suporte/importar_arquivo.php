...
foreach ($dados as $linha) {
	...
	if(strlen($valor[3]) == 0){ $hora_updt = null; }else{ $hora_updt = $valor[3]; }	// Hora de atualização
	if(strlen($valor[4]) == 0){ $verificado = null; }else{ $verificado = $valor[4]; } 	// Verificação
	if(strlen($valor[5]) == 0){ $categoria = null; }else{ $categoria = $valor[5]; }     // Categoria
	if(strlen($valor[6]) == 0){ $dia_marcado = null; }else{ $dia_marcado = $valor[6]; } // Nome do dia
	if($valor[7] == ""){ $nome_imagem = null; }else{ $nome_imagem = $valor[7]; }		// Nome da imagem
	if($valor[8] == ""){ $descricao = null; }else{ $descricao = $valor[8]; }			// Descrição do dia
